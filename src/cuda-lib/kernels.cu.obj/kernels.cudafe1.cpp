...
#pragma GCC diagnostic pop
# 7 "../kernels.cu"
void cuda_kernel_warmup(float *p) ;
#if 0
# 8
{ 
# 9
size_t idx = ((__device_builtin_variable_blockIdx.x) * (__device_builtin_variable_blockDim.x)) + (__device_builtin_variable_threadIdx.x); 
# 10
float f = (float)idx; 
# 11
(p[idx]) = ((f * f) * f); 
...
#endif
# 15 "../kernels.cu"
void cuda_kernel_sgemm_0(float *
# 16
a, float *b, float *c, size_t 
# 17
N, size_t M, size_t K, float 
# 18
alpha, float beta) ;
#if 0
# 19
{ 
# 20
int ir = ((__device_builtin_variable_blockIdx.x) * (32)) + (__device_builtin_variable_threadIdx.x); 
# 21
int ic = ((__device_builtin_variable_blockIdx.y) * (32)) + (__device_builtin_variable_threadIdx.y); 
# 23
if ((ir < M) && (ic < N)) 
# 24
{ 
...
{ 
# 29
acc += ((a[(ir * K) + k]) * (b[(k * N) + ic])); 
# 30
}  
# 31
(c[(ir * N) + ic]) = ((alpha * acc) + (beta * (c[(ir * N) + ic]))); 
# 33
}  
# 34
} 
#endif
# 37 "../kernels.cu"
void cuda_kernel_sgemm_1(float *
# 38
a, float *b, float *c, size_t 
# 39
N, size_t M, size_t K, float 
...
{ 
# 42
int tr = __device_builtin_variable_threadIdx.x; 
# 43
int tc = __device_builtin_variable_threadIdx.y; 
# 44
int ir = ((__device_builtin_variable_blockIdx.x) * (32)) + (__device_builtin_variable_threadIdx.x); 
# 45
int ic = ((__device_builtin_variable_blockIdx.y) * (32)) + (__device_builtin_variable_threadIdx.y); 
# 47
__attribute__((unused)) static float a_sub[32][32]; 
# 48
__attribute__((unused)) static float b_sub[32][32]; 
# 50
int load_size = K / (32); 
# 51
if ((K % (32)) != (0)) 
...
if ((a_ir < M) && (a_ic < K)) { 
# 66
((a_sub[tr])[tc]) = (a[(a_ir * K) + a_ic]); }  
# 67
if ((b_ir < K) && (b_ic < N)) { 
# 68
((b_sub[tr])[tc]) = (b[(b_ir * N) + b_ic]); }  
# 70
__syncthreads(); 
# 73
#pragma unroll
...
#endif
# 87 "../kernels.cu"
void cuda_kernel_sgemm_2(float *
# 88
a, float *b, float *c, size_t 
# 89
N, size_t M, size_t K, float 
...
int ic = ((__device_builtin_variable_blockIdx.y) * (32)) + (__device_builtin_variable_threadIdx.y); 
# 97
__attribute__((unused)) static float a_sub[32][32 + 1]; 
# 98
__attribute__((unused)) static float b_sub[32][32 + 1]; 
# 100
int load_size = K / (32); 
# 101
if ((K % (32)) != (0)) 
...
if ((a_ir < M) && (a_ic < K)) { 
# 116
((a_sub[tr])[tc]) = __ldg(&(a[(a_ir * K) + a_ic])); }  
# 117
if ((b_ir < K) && (b_ic < N)) { 
# 118
((b_sub[tr])[tc]) = __ldg(&(b[(b_ir * N) + b_ic])); }  
# 120
__syncthreads(); 
# 123
#pragma unroll
...
#define _NV_ANON_NAMESPACE _GLOBAL__N__15_kernels_cpp1_ii_d5ba3146
# 1 "kernels.cudafe1.stub.c"
#include "kernels.cudafe1.stub.c"
# 1 "kernels.cudafe1.stub.c"
