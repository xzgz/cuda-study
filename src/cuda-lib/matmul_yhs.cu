...
static void random_init(float* data, size_t size) {
...
__device__ __noinline__ void C_tile_wb(StgFrag C_frag, float* C_stg_ptr, const float* C_lds_ptr, uint32_t C_sts_addr,
 ...
 *
 */
__global__ __launch_bounds__(256) void ampere_sgemm_128x256x8_kernel(const float* A, const float* B, float* C, size_t m,
        size_t n, size_t k, size_t B_ldg_step,
...
#pragma unroll
        for (int i = 0; i < 4; ++i) {
            ...
            ldgsts32(B_sts_addr + (i * 2 * 256 + 128) * sizeof(float),
                    B_ldg_ptr + B_ldg_offset[i] + 128 * sizeof(float), src_size, (B_ldg_guard & (1u << 1)) != 0);
        }
