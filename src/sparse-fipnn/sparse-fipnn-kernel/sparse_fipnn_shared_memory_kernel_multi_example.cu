...
int32_t* gmem_fw_field_map, T* common_output) {
...
T* gmem_common_cross_mean_sum, T* gmem_common_cross_square_sum,
int32_t* gmem_common_field_map, const T* weight_tensor,
const int32_t* field_tensor, const T* common_output, T* output_tensor,
T* workspace) {
    extern __shared__ float smem_pool[];
    int32_t batch_id = blockIdx.x;
...
namespace functor {
    ...
    int32_t ComputeSparseFipnnSharedMemory(cudaStream_t stream, const void* const* input, T* output,
        ...
        data_offset = (data_offset + data_align_count - 1) / data_align_count * data_align_count;
        T* common_output = reinterpret_cast<T*>(gmem_field_map + data_offset);

        data_offset = (fw_weight_size + data_align_count - 1) / data_align_count * data_align_count;
        T* gmem_cross_sum = common_output + data_offset;
        ...
        if ((max_embedding_segment_size > 0) && (embedding_size >= 32)) {
            ComputeBatchBoundary<<<DIVUP(sample_feature_size, 1024), block_boundary, 0, stream>>>(
            index_tensor, sample_feature_size, batch_size, sample_feature_start_addr, fw_weight_size, common_output);
            ...
            sample_feature_start_addr, weight_tensor, field_tensor, gmem_common_cross_mean_sum, gmem_field_map, common_output);
            ...
            field_tensor, common_output, output, gmem_cross_sum);

        } else if (embedding_size <= 192) {
            ComputeBatchBoundary<<<DIVUP(sample_feature_size, 1024), block_boundary, 0, stream>>>(
            index_tensor, sample_feature_size, batch_size, sample_feature_start_addr, 0, output);
    ...
    const int32_t embedding_size) {
        const void **single_example_input = new const void*[3];
        for (int32_t i = 0; i < 3; ++i) single_example_input[i] = nullptr;
        T *single_example_output = nullptr;
        int32_t sample_count = 0;
        int32_t batch_size = 0;
        int32_t acc_sample_count = 0;
        int32_t acc_batch_size = 0;
        int32_t feature_weight_offset = field_num * embedding_size;
        int32_t fw_weight_size = fw_field_num * (fw_field_num + 1) / 2;
        for (int32_t i = 0; i < example_count; ++i) {
            sample_count = sample_count_list[i];
            batch_size = batch_size_list[i];
            single_example_input[0] = static_cast<const T*>(input[0]) + acc_sample_count * feature_weight_offset;
            single_example_input[1] = static_cast<const int32_t*>(input[1]) + acc_sample_count * field_num;
            single_example_input[2] = static_cast<const int32_t*>(input[2]) + acc_sample_count;
            single_example_output = output + acc_batch_size * fw_weight_size;

            ComputeSparseFipnnSharedMemory(
            stream, single_example_input, single_example_output, workspace, fw_field_num, sample_count, field_num,
            embedding_size, batch_size);

            acc_sample_count += sample_count;
            acc_batch_size += batch_size;
        }

        delete [] single_example_input;
        return 1;
    }
