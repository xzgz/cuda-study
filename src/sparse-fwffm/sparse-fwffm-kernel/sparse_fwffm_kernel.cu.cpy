...

{
    ...
    ProcessOutput<T, warp_num>(mem_cross_mean_sum, mem_cross_square_sum, mem_fw_cross_mean_sum,
                               ...
                               fw_weight_tensor, output_tensor, batch_id, embedding_size, field_num,
                               fw_field_num);
}
...
int32_t ComputeSparseFwffm(cudaStream_t stream, const void* const* input, T* output,
    ...
    T* gmem_fw_cross_mean_sum =
        gmem_cross_sum + batch_size * (field_num + 1) * embedding_size * fw_field_num;
    T* gmem_fw_cross_square_sum =
        gmem_fw_cross_mean_sum + embedding_size * field_num * fw_field_num;

    int32_t* gmem_fw_field_map =
        reinterpret_cast<int*>(gmem_fw_cross_square_sum + embedding_size * fw_field_num);

    CUDA_CHECK(cudaMemset(gmem_fw_cross_mean_sum, 0,
                          sizeof(float) * (embedding_size * field_num * fw_field_num +
                                           embedding_size * fw_field_num)));
    CUDA_CHECK(cudaMemset(gmem_fw_field_map, -1, sizeof(int) * (fw_field_num)));
    CUDA_CHECK(cudaMemset(output, 0, sizeof(float) * (batch_size * embedding_size)));

    CUDA_CHECK(cudaMemset(sample_feature_start_addr, 0, sizeof(int32_t) * (batch_size + 1)));
    CUDA_CHECK(cudaMemset(sample_feature_end_addr, 0, sizeof(int32_t) * (batch_size + 1)));
...
//     } else {
        ...
        int32_t shared_mem_required_bytes = fw_field_num * 3 * sizeof(int32_t);
        dim3 grid(batch_size);
