...
int32_t* sample_feature_start_addr, int32_t output_size, T* output) {
...
#pragma unroll
    ...
    for (int32_t i = tid; i < output_size; i += total_thread) {
        int32_t col = i % embedding_size;
        if (col >= embedding_segment_start && col < embedding_segment_start + embedding_segment_size) {
            atomicAdd(output + i, smem_output[col - embedding_segment_start]);
        }
    }
}
