...
int main(int argc, char **argv)
{
  ...
  printf("d_B address: %p\n", d_B);
  // free device global memory
  CHECK(cudaFree(d_A));
  CHECK(cudaFree(d_B));
  printf("After cudaFree d_A address: %p\n", d_A);
  printf("After cudaFree d_B address: %p\n", d_B);

  printf("h_A address: %p\n", h_A);
  printf("h_B address: %p\n", h_B);
  // free host memory
  free(h_A);
  free(h_B);
  printf("After free h_A address: %p\n", h_A);
  printf("After free h_B address: %p\n", h_B);

  // part 2: using zerocopy memory for array A and B
  // allocate zerocpy memory
  CHECK(cudaHostAlloc((void **)&h_A, nBytes, cudaHostAllocMapped));
  CHECK(cudaHostAlloc((void **)&h_B, nBytes, cudaHostAllocMapped));
  printf("After cudaHostAlloc h_A address: %p\n", h_A);
  printf("After cudaHostAlloc h_B address: %p\n", h_B);
  ...
  printf("After cudaHostGetDevicePointer d_A address: %p\n", d_A);
  printf("After cudaHostGetDevicePointer d_B address: %p\n", d_B);
